...
quantity1.select_by_index(2)
quantity2=Select(driver.find_element(By.XPATH,"//tbody/tr[2]/td[3]/select[1]"))
quantity2.select_by_index(3)
quantity3=Select(driver.find_element(By.XPATH,"//tbody/tr[2]/td[3]/select[1]"))
quantity3.select_by_index(4)

time.sleep(2)
l = len(driver.find_elements(By.CSS_SELECTOR, "tbody tr"))
Price_List=[]
for r in range(l,l-2):
    Var=driver.find_element(By.CSS_SELECTOR,"tbody tr:nth-child(+ str +) td:nth-child(4)").text
    Product_Price=(Var[1:])
    Price_List.append(float(Product_Price))
var2=sum(Price_List)
Expected_subtotal=round(var2,2)
print(Expected_subtotal)
Exp_Tax=round((Expected_subtotal*0.13),2)
print(Exp_Tax)
Exp_Total=Expected_subtotal+Exp_Tax
print(Exp_Total)
...
if Expected_subtotal == Act_SubTotal:
